...
void Game::handleEvents()
{
    ...
    else if (game_state == 1)
    {
        while(SDL_PollEvent( &Game::event ) != 0)
        {
            ...
            if( currentKeyStates[ SDL_SCANCODE_SPACE ] && !glb.player_power_invisble && glb.player_power_invisble_count > 0)
            {
                player.setAlpha(150);
                glb.player_power_invisble = true; 
                player_invisble_time = SDL_GetTicks();
                glb.player_power_invisble_count--;
...
void Game::update()
{
    ...
    else if (game_state == 1)
    {
        ...
        glb.enemy1.move(tileSet, wallSet);
        glb.enemy2.move(tileSet, wallSet);
        glb.enemy3.move(tileSet, wallSet);
        glb.enemy4.move(tileSet, wallSet);

        if ((SDL_GetTicks() - player_invisble_time >= 3000) && glb.player_power_invisble)
        {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (checkCollision(playerCollider, *enemies[i]) && !glb.player_power_invisble 
                && !(glb.player_X == 50 && glb.player_Y == 50 ) )
            {
                glb.player_X = 50;
                glb.player_Y = 50;
    ...
    else if (game_state == 3)
    {
        ...
        if (SDL_GetTicks() - state_3_time >= etime)
        {
            ...
            glb.mazefile = savebmp();
            glb.mazefile = addCoins(glb.mazefile, initial_coins1, initial_coins2);
